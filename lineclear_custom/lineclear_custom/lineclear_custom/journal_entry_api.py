import frappe, os, math
import frappe.utils.pdf
from frappe.utils import money_in_words, getdate


@frappe.whitelist()
def download_entry(doc_no):
    """
    Download the Journal Entry / Bank Entry / Cash Entry as a PDF.
    """
    if not doc_no:
        frappe.throw("Document number is required to download the entry.")
    
    # clear cache for this document to ensure fresh data
    frappe.clear_document_cache("Journal Entry", doc_no)

    doc, letter_head, customer, address, contact = get_metadata(doc_no)
    delimiter = '<div style="page-break-before: always;"></div>'
    money2words = money_in_words(doc.net_total).upper() if doc.net_total else "ZERO"
    template = frappe.get_template("lineclear_custom/public/template/journal_entry.html")

    batch_size = 30
    footer_size = 12

    # calculate total pages properly
    total_rows = sum(rows_taken(item) for item in doc.accounts) + footer_size
    total_pages = math.ceil(total_rows / batch_size) or 1

    rendered_html_chunks = []

    item_index = 0

    for page in range(1, total_pages + 1):
        is_last = page == total_pages
        current_batch = []
        rows_used = 0

        while item_index < len(doc.accounts):
            item = doc.accounts[item_index]
            row_cost = rows_taken(item)
            item.row_cost = row_cost

            if item.credit > 0:
                item_index += 1
                continue

            # If adding this item exceeds the page row limit, break and move to next page
            if rows_used + row_cost > (batch_size - (footer_size if is_last else 0)):
                break

            current_batch.append(item)
            rows_used += row_cost
            item_index += 1

        rendered_chunk = template.render({
                "statement_date": getdate(doc.posting_date).strftime("%d/%m/%Y"),
                "letter_head": letter_head,
                "invoice_no": doc_no,
                "total_pages": total_pages,
                "page": page,
                "statements": current_batch,
                "money2words": money2words[4:],
                "doc": doc,
                "footer": is_last,
                "footer_size": footer_size,
        })
        rendered_html_chunks.append(rendered_chunk)

    # join all chunks with page breaks between them
    full_html = delimiter.join(rendered_html_chunks)

    # generate PDF
    rendered_pdf = frappe.utils.pdf.get_pdf(full_html)

    # send the PDF as a response
    frappe.local.response.filename = f"{doc_no}.pdf"
    frappe.local.response.filecontent = rendered_pdf
    frappe.local.response.type = "download"


@frappe.whitelist()
def download_note(doc_no):
    """
    Download the credit / debit notes as a PDF.
    """
    if not doc_no:
        frappe.throw("Document number is required to download the note.")
    
    # clear cache for this document to ensure fresh data
    frappe.clear_document_cache("Journal Entry", doc_no)

    doc, letter_head, customer, address, contact = get_metadata(doc_no)
    delimiter = '<div style="page-break-before: always;"></div>'
    money2words = money_in_words(doc.net_total).upper() if doc.net_total else "ZERO"

    if doc.voucher_type == 'Debit Note':
        template = frappe.get_template("lineclear_custom/public/template/debit_note.html")
    if doc.voucher_type == 'Credit Note':
        template = frappe.get_template("lineclear_custom/public/template/credit_note.html")

    batch_size = 30
    footer_size = 12

    # calculate total pages properly
    total_rows = sum(rows_taken(item) for item in doc.custom_accounting_entires) + footer_size
    total_pages = math.ceil(total_rows / batch_size) or 1

    rendered_html_chunks = []

    item_index = 0

    for page in range(1, total_pages + 1):
        is_last = page == total_pages
        current_batch = []
        rows_used = 0

        while item_index < len(doc.custom_accounting_entires):
            item = doc.custom_accounting_entires[item_index]
            row_cost = rows_taken(item)
            item.row_cost = row_cost

            # If adding this item exceeds the page row limit, break and move to next page
            if rows_used + row_cost > (batch_size - (footer_size if is_last else 0)):
                break

            current_batch.append(item)
            rows_used += row_cost
            item_index += 1

        rendered_chunk = template.render({
                "statement_date": getdate(doc.posting_date).strftime("%d/%m/%Y"),
                "letter_head": letter_head,
                "invoice_no": doc_no,
                "customer": customer,
                "address": address,
                "contact": contact,
                "total_pages": total_pages,
                "page": page,
                "statements": current_batch,
                "money2words": money2words[4:],
                "doc": doc,
                "footer": is_last,
                "footer_size": footer_size,
                "company": str(doc.company).upper(),
        })
        rendered_html_chunks.append(rendered_chunk)

    # join all chunks with page breaks between them
    full_html = delimiter.join(rendered_html_chunks)

    # generate PDF
    rendered_pdf = frappe.utils.pdf.get_pdf(full_html)

    # send the PDF as a response
    frappe.local.response.filename = f"{doc_no}.pdf"
    frappe.local.response.filecontent = rendered_pdf
    frappe.local.response.type = "download"


def get_metadata(doc_no):
    doc = frappe.get_doc("Journal Entry", doc_no)

    letter_head = frappe.db.get_value(
        'Letter Head', 
        doc.letter_head,
        fieldname=['content']
    )

    if doc.custom_qr_code_link:
            doc.custom_qr_code_link = doc.custom_qr_code_link.replace('api.', '')

    if len(doc.custom_accounting_entires) > 0:
        if doc.custom_accounting_entires:
            for entry in doc.custom_accounting_entires:
                doc.net_total = entry.amount + entry.tax_amount

        customer = frappe.db.get_value(
            'Customer',
            doc.customer,
            fieldname=['debtor_code', 'customer_name', 'payment_terms', 'sst_exemption', 'custom_registration_type', 'custom_registration_no', 'tax_id', 'customer_primary_address', 'customer_primary_contact'],
            as_dict=1
        )
        address = frappe.db.get_value(
            'Address',
            customer.customer_primary_address,
            fieldname=['unit_number', 'address_line1', 'address_line2', 'city', 'state'],
            as_dict=1
        )

        contact = {}
        # Phone
        if customer.mobile_no:
            contact['phone'] = customer.mobile_no
        else:
            phone = frappe.db.get_value(
                'Contact Phone',
                filters={'parent': customer.customer_primary_contact},
                fieldname='phone'
            )
            contact['phone'] = phone or ''

        # Email
        if customer.email_id:
            contact['email'] = customer.email_id
        else:
            email = frappe.db.get_value(
                'Contact Email',
                filters={'parent': customer.customer_primary_contact},
                fieldname='email_id'
            )
            contact['email'] = email or ''

    elif len(doc.accounts) > 0:
        doc.net_total = 0

        for entry in doc.accounts:
            doc.net_total += entry.debit

        customer = None
        address = None
        contact = None

    return doc, letter_head, customer, address, contact


def execute_sql_from_file(file_name, values=None, as_dict=False):
    """
        execute SQL query from a file with optional parameters
    """
    filepath = os.path.join(frappe.get_app_path('lineclear_custom'), 'public', 'sql', file_name)
    
    if not isinstance(values, (list, tuple)):
        values = (values,)
    
    with open(filepath, 'r') as f:
        queries = f.read().split(';')

    results = []

    for query in queries:
        query = query.strip()
        if not query:
            continue
            
        # Special handling for IN clause with a tuple of values
        if "IN (%s)" in query:
            # Create placeholders for each item in the tuple
            placeholders = ', '.join(['%s'] * len(values))
            # Replace the single %s with the correct number of placeholders
            query = query.replace("IN (%s)", f"IN ({placeholders})")
            # Use the actual tuple values directly
            try:
                result = frappe.db.sql(query, values=values, as_dict=as_dict)
                results.append(result)
            except Exception as e:
                frappe.log_error(f"SQL Error: {str(e)}\nQuery: {query}\nValues: {values}", "SQL Execution Error")
                raise
            continue
    
        if '%s' in query:
            try:
                result = frappe.db.sql(query, values=values, as_dict=as_dict)
                results.append(result)
            except Exception as e:
                frappe.log_error(f"SQL Error: {str(e)}\nQuery: {query}\nValues: {values}", "SQL Execution Error")
                raise
        else:
            if query:
                try:
                    result = frappe.db.sql(query, as_dict=as_dict)
                    results.append(result)
                except Exception as e:
                    frappe.log_error(f"SQL Error: {str(e)}\nQuery: {query}", "SQL Execution Error")
                    raise
    
    return results[-1]


def rows_taken(item, max_chars=40):
    description = item.get("description") or item.get("custom_description") or ""
    length = len(description) if description else 0

    return max(1, math.ceil(length / max_chars))