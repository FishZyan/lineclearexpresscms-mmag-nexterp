...
def import_apdebit():
    ...
    logger = frappe.logger("apdebit_import")
    failed_docs, failed_rows = [], []

    # filepath = os.path.join(frappe.get_app_path("lineclear_custom"), "lineclear_custom", "source", "apdebit_20250331.xlsx")
    # filepath = os.path.join(frappe.get_app_path("lineclear_custom"), "lineclear_custom", "apdebit_error_fixed.xlsx")
    filepath = os.path.join(frappe.get_app_path("lineclear_custom"), "lineclear_custom", "apdebit_patch.xlsx")
    ...
    for invoice_id, group in grouped:
        ...
        try:
            ...
            # SPECIAL CASE: Rebate 
            if doc.name in ('DN-000085', 'DN-000189', 'DN-000195', 'DN-000196'):
            ...
            failed_docs.append(invoice_id)
            failed_rows.append(group.copy())
            logger.error(f"❌ Failed to import debit {invoice_id}: {e}", exc_info=True)
    ...
    if failed_docs:
        print(f"⚠️ Import completed with errors. Failed debits: {failed_docs}")
        failed_df = pd.concat(failed_rows)
        failed_filepath = os.path.join(frappe.get_app_path("lineclear_custom"), "lineclear_custom", "error", "apdebit_error.xlsx")
        failed_df.to_excel(failed_filepath, index=False)
    else:
        print("✅ All debit notes imported successfully.")
