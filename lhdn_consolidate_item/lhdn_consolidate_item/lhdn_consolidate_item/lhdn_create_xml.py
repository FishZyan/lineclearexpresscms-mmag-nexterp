...
# Function to grab date range and save into cache for later use
def update_date_range(date_value,batch_id):
    new_date = getdate(date_value)
    cached = frappe.cache().get_value(batch_id) or {
        "start_date":  None,
        "end_date": None
    }

    if not cached["start_date"] or new_date < cached["start_date"]:
        cached["start_date"] = new_date

    if not cached["end_date"] or new_date > cached["end_date"]:
        cached["end_date"] = new_date

    frappe.cache().set_value(batch_id, cached)
...
def custom_invoice_data(invoice,invoice_number_list, doctype_data,batch_id):
    try:
        print("enter in custom consolidate invoice")
                
        cbc_ID = ET.SubElement(invoice, "cbc:ID")   #initialize
        cbc_ID.text = str(batch_id)  # assign
...
def consolidate_supplier_Data(invoice): # Supplier Data for Self bill related invoice/credit note/debit note
    try:
                ...
                cbc_IndustryClassificationCode.text = "00000"
                ...
                cbc_ID_2.text = "EI00000000010"
                    
                    

                #BRN    
                # Supplier’s Registration / Identification Number / Passport Number
                supplier_id_type = "BRN"  # Example field to determine the type of ID
                supplier_id_number = "NA" # Example field for the ID number
                ...
                cac_PartyLegalEntity_1 = ET.SubElement(cac_Party_1, "cac:PartyLegalEntity")
                cbc_RegistrationName_1 = ET.SubElement(cac_PartyLegalEntity_1, "cbc:RegistrationName")
                # For Consolidate Item, the registration name will be always hard coded Consolidate Buyers
                cbc_RegistrationName_1.text = "General Public"
...
def company_Data_customer(invoice,consolidate_invoice_doc, doctype_data):
    try:
        company_doc = frappe.get_doc("Company", consolidate_invoice_doc.company)       
        address_name = frappe.get_value("Dynamic Link", {
                    "link_doctype": "Company",
                    "link_name": consolidate_invoice_doc.company,
                    "parenttype": "Address"
                }, "parent") 
        address_list = frappe.get_list(
            "Address", 
            filters={"is_your_company_address": "1", "name": address_name}, 
        ...
        # Company Malaysia Standard Industrial Classification (MSIC) Code
        #/ ubl:Invoice / cac:AccountingSupplierParty / cac:Party / cbc:IndustryClassificationCode
        cbc_IndustryClassificationCode = ET.SubElement(cac_Party_2, "cbc:IndustryClassificationCode")
        ...
        # Company Tin Code
        cac_PartyIdentification = ET.SubElement(cac_Party_2, "cac:PartyIdentification")
        cbc_ID_2 = ET.SubElement(cac_PartyIdentification, "cbc:ID")
        cbc_ID_2.set("schemeID", "TIN")
        cbc_ID_2.text = str(company_doc.tax_id )

        #BRN    
        # Company Registration / Identification Number / Passport Number
        company_id_type = company_doc.custom_registration_type  # Example field to determine the type of ID
        company_id_number = company_doc.company_registration  # Example field for the ID number
        cac_PartyIdentification_1 = ET.SubElement(cac_Party_2, "cac:PartyIdentification")
        cbc_ID_Identification = ET.SubElement(cac_PartyIdentification_1, "cbc:ID")
        if company_id_type == 'BRN':
            cbc_ID_Identification.set("schemeID", "BRN")
        elif company_id_type == 'NRIC':
            cbc_ID_Identification.set("schemeID", "NRIC")
        elif company_id_type == 'PASSPORT':
            cbc_ID_Identification.set("schemeID", "PASSPORT")
        elif company_id_type == 'ARMY':
            cbc_ID_Identification.set("schemeID", "ARMY")
        cbc_ID_Identification.text = str(company_id_number)

        # Company's SST Registration Number
        #/ ubl:Invoice / cac:AccountingSupplierParty / cac:Party / cac:PartyIdentification / cbc:ID [@schemeID=’SST’]
        ...
        # Company Tourism Tax Registration Number
        #/ ubl:Invoice / cac:AccountingSupplierParty / cac:Party / cac:PartyIdentification / cbc:ID [@schemeID=’TTX’]
        ...
        cac_PartyLegalEntity = ET.SubElement(cac_Party_2, "cac:PartyLegalEntity")
        ...
        cbc_Telephone = ET.SubElement(cac_Contact, "cbc:Telephone")
        cbc_Telephone.text = company_doc.custom_contact_no.replace('-','') # remove all "-" and final result will be only digit number
...
# Company Data
def company_Data(invoice,consolidate_invoice_doc, doctype_data): #Company data
            try:
                company_doc = frappe.get_doc("Company", consolidate_invoice_doc.company)
                address_name = frappe.get_value("Dynamic Link", {
                    "link_doctype": "Company",
...
def consolidate_customer_Data(invoice):
            try:
                ...
                # Customer's Malaysia Standard Industrial Classification (MSIC) Code
                #/ ubl:Invoice / cac:AccountingSupplierParty / cac:Party / cbc:IndustryClassificationCode
                cbc_IndustryClassificationCode = ET.SubElement(cac_Party_2, "cbc:IndustryClassificationCode")
                cbc_IndustryClassificationCode.text = "00000"
                ...
                cac_PartyIdentification_3 = ET.SubElement(cac_Party_2, "cac:PartyIdentification")
                cbc_ID_SST_1 = ET.SubElement(cac_PartyIdentification_3, "cbc:ID")
                cbc_ID_SST_1.set("schemeID", "SST")
                cbc_ID_SST_1.text = "NA"

                # Customer TTX
                cac_PartyIdentification_4 = ET.SubElement(cac_Party_2, "cac:PartyIdentification")
                cbc_ID_TTX_1 = ET.SubElement(cac_PartyIdentification_4, "cbc:ID")
                cbc_ID_TTX_1.set("schemeID", "TTX")
                cbc_ID_TTX_1.text = "NA"
...
def tax_Data(invoice,json_invoice_list,consolidate_invoice_doc,total_local_tax, total_taxable_amount, total_final_amount,rounding_total):
    try:
                ...
                cbc_LineExtensionAmount.set("currencyID", consolidate_invoice_doc.currency)
                cbc_LineExtensionAmount.text = str(total_taxable_amount)
                ...
                # Allowance total amount always zero
                # Discount Amount if applicable
                cbc_AllowanceTotalAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:AllowanceTotalAmount")
                cbc_AllowanceTotalAmount.set("currencyID", consolidate_invoice_doc.currency)
                cbc_AllowanceTotalAmount.text = "0"

                # Charge Total Amount always zero
                # Extra charging if applicable, for example shipping
                cbc_ChargeTotalAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:ChargeTotalAmount")
                cbc_ChargeTotalAmount.set("currencyID", consolidate_invoice_doc.currency)
                cbc_ChargeTotalAmount.text = "0"

                # Payable Rounding Amount always zero
                # Any rounding if applicable
                cbc_PayableRoundingAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:PayableRoundingAmount")
                cbc_PayableRoundingAmount.set("currencyID", consolidate_invoice_doc.currency)
                cbc_PayableRoundingAmount.text = str(rounding_total) 

                # Total Payable Amount
                # Final payable amount when there is discount or rounding or extra charges
...
def item_data_manual(invoice, json_invoice_list,batch_id, doctype_data):
    try:
        # Intialize variable
        consolidate_item_list = frappe.get_list(doctype_data, filters=[["name", "in", json_invoice_list]], fields=["name", "tax", "sub_total_ex", "total","tax_percentage", "currency","invoice_date","tax_exemption_reason","lhdn_tax_code"])
        running_number = 1

        for item in consolidate_item_list: 
            tax_percentage_value = item.tax_percentage
            tax = item.tax
            sub_total_ex = item.sub_total_ex
            currency = item.currency
            item_description = item.name
            item_date_value = item.invoice_date
            quantity = "1" # Manual import, quanity will always be 1
            single_unit_price = item.sub_total_ex
            TaxExemptionReason = item.tax_exemption_reason
            if item.lhdn_tax_code != None or item.lhdn_tax_code != "":
                tax_category_code = item.lhdn_tax_code # Manual Import, tax category code will be 01 Sales Tax
            else:
                tax_category_code = "01" # Manual Import, tax category code will be 01 Sales Tax

            # Update Date Range for Summary report purpose
            update_date_range(item_date_value, batch_id)
            ...
            cbc_InvoicedQuantity.text = quantity  
            ...
            cbc_LineExtensionAmount_1.text = str(sub_total_ex)     #including only charges or discount and exclsing tax
            ...
            cbc_TaxAmount_1 = ET.SubElement(cac_TaxTotal, "cbc:TaxAmount")
            cbc_TaxAmount_1.set("currencyID", currency)   #Tax Amount of each item
            cbc_TaxAmount_1.text = str(tax)  
            ...
            cbc_Description.text = str(item_description)
            ...
            cbc_PriceAmount.text = str(single_unit_price)
            ...
            cbc_Amount.set("currencyID", currency)
            cbc_Amount.text = str(sub_total_ex)
...
def item_data_system(invoice, json_invoice_list,batch_id, doctype_data):
    try:
        # Intialize variable
        consolidate_item_list = frappe.get_list(doctype_data, filters=[["name", "in", json_invoice_list]], fields=["name", "currency", "posting_date"])
        running_number = 1

        for item in consolidate_item_list: 
            item_date_value = item.posting_date
            parent_item_doc = frappe.get_doc(doctype_data, item.name)
            child_item_list = None
            if doctype_data == sales_invoice_doctype:
            ...
            for child_item in child_item_list:
                # Initialize Variable for to handle different doctype

                check_flag_tax_code = False # Default No tax code
                tax_percentage_value = 0.0
                tax = 0
                sub_total_ex = 0
                currency = item.currency 
                item_description = ""
                ...
                if doctype_data == sales_invoice_doctype:
                    if child_item.custom_tax_code:
                        tax_template = frappe.get_doc(sales_taxes_and_charges_template_doctype, child_item.custom_tax_code)
                        if len(tax_template.taxes) > 0: # handle tax rate like ESV-6 without any tax rate
                            tax_rate = tax_template.taxes[0].rate
                            tax_percentage_value = float(tax_rate)
                            check_flag_tax_code = True
                    tax = child_item.custom_tax_amount
                    sub_total_ex = child_item.net_amount
                    item_description = child_item.item_name
                    quantity = child_item.qty
                    single_unit_price = child_item.rate
                    if child_item.custom_lhdn_tax_type_code:
                        tax_category_code = child_item.custom_lhdn_tax_type_code
                    else:
                        tax_category_code = '02'
                    TaxExemptionReason = child_item.custom_exemption_description_

                    if TaxExemptionReason == "" or TaxExemptionReason == None:
                         TaxExemptionReason = 'Exempted Company'
                elif doctype_data == journal_entry_doctype:
                    if child_item.tax_code:
                        tax_template = frappe.get_doc(sales_taxes_and_charges_template_doctype, child_item.tax_code)
                        if len(tax_template.taxes) > 0: # handle tax rate like ESV-6 without any tax rate
                            tax_rate = tax_template.taxes[0].rate
                            tax_percentage_value = float(tax_rate)
                            check_flag_tax_code = True
                    tax = child_item.tax_amount
                    sub_total_ex = child_item.amount
                    item_description = child_item.description
                    quantity = "1" # credit note and debit note, item quantity always one
                    single_unit_price = child_item.amount # credit note and debit note, item always euqal to amount
                    if child_item.lhdn_tax_type_code:
                        tax_category_code = child_item.lhdn_tax_type_code
                    else:
                        tax_category_code = '02'
                    TaxExemptionReason = child_item.exemption_description
                ...
                cbc_InvoicedQuantity = ET.SubElement(cac_InvoiceLine, "cbc:InvoicedQuantity")
                cbc_InvoicedQuantity.set("unitCode","C62")
                cbc_InvoicedQuantity.text = str(quantity)
                ...
                # Tax Rate in percentage
                cbc_TaxRatePercent = ET.SubElement(cac_TaxSubtotal, "cbc:Percent")
                if check_flag_tax_code: # If there is tax percentage value
                    cbc_TaxRatePercent.text = str(abs(round(float(tax_percentage_value)/100, 2)))
                else:
                    cbc_TaxRatePercent.text = "0"
                  
                cac_TaxCategory = ET.SubElement(cac_TaxSubtotal, "cac:TaxCategory")            
                
                cbc_TaxCategoryID = ET.SubElement(cac_TaxCategory, "cbc:ID")
                cbc_TaxCategoryID.text = str(tax_category_code)
        ...
        print(f"Error occurred in item data: " + str(e))
        raise RuntimeError(f"Error occurred in item data: " + str(e))
...
def calculate_consolidate_amount(invoice_number_list, doctype_data):

    # Initialize
    invoice_item_list = []

    if doctype_data == lhdn_submission_doctype:
        invoice_item_list = frappe.get_list(doctype_data, filters=[["name", "in", invoice_number_list]], fields=["name", "tax", "sub_total_ex", "total"])
    elif doctype_data == sales_invoice_doctype:
        invoice_item_list = frappe.get_list(doctype_data, filters=[["name", "in", invoice_number_list]], fields=["name", "total_taxes_and_charges", "total", "grand_total","rounded_total","rounding_adjustment"])  
    elif doctype_data == journal_entry_doctype:
        invoice_item_list = frappe.get_list(doctype_data, filters=[["name", "in", invoice_number_list]], fields=["name","custom_total_tax_amount","total_amount"])
    elif doctype_data == purchase_invoice_doctype:
        invoice_item_list = frappe.get_list(doctype_data, filters=[["name", "in", invoice_number_list]], fields=["name", "total_taxes_and_charges", "total", "grand_total","rounded_total", "rounding_adjustment"])  
    ...
    # Erpnext Sales Invoice Doctype
        ...
        for single_item in invoice_item_list:
            ...
            rounding_total += single_item.rounding_adjustment
    # Erpnext Journal Entry Doctype (Credit Note and Debit Note)
    elif doctype_data == journal_entry_doctype:
        for single_item in invoice_item_list:
    ...
    # Erpnext Purchase Invoice (self-bill invoice)
    elif doctype_data == purchase_invoice_doctype:
        for single_item in invoice_item_list:
            total_tax += single_item.total_taxes_and_charges
            total_taxable_amount += single_item.total
    ...
    rounding_total = round(abs(rounding_total), 2)

    return total_tax, total_taxable_amount, total_final_amount, rounding_total
...
def billing_Reference_data(invoice, document_type):
    
    # Get Last UUID which is Valid
    # Using this Latest UUID to submit CN and DN for consolidate
    if document_type == "Credit Note" or document_type == "Debit Note":
        lhdn_consolidate_summary = frappe.get_list(lhdn_summary_doctype, filters={"lhdn_status": "Valid", "document_type": "Invoice"}, fields=["name", "uuid"], order_by="creation desc", limit=1)
    else:
        lhdn_consolidate_summary = frappe.get_list(lhdn_summary_doctype, filters={"lhdn_status": "Valid", "document_type": "Self-billed Invoice"}, fields=["name", "uuid"], order_by="creation desc", limit=1)
    ...
    cbc_billingReferenceID = ET.SubElement(cac_InvoiceDocumentReference, "cbc:ID")
    cbc_billingReferenceID.text = str(lhdn_consolidate_summary[0].name)
    cbc_billingReferenceUUID = ET.SubElement(cac_InvoiceDocumentReference, "cbc:UUID")
    cbc_billingReferenceUUID.text = str(lhdn_consolidate_summary[0].uuid)
