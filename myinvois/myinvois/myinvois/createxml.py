...
def company_Data(invoice,sales_invoice_doc): #supplier data
    try:
        ...
        if len(address_name) == 0:
            frappe.msgprint("LHDN requires proper address. Please add your company address in address master")
            frappe.log_error("LHDN requires proper address. Please add your company address in address master")
            return
        ...
        if(not company_doc.custom_registration_type):
            frappe.msgprint("LHDN requires proper Registration Type." \
                    "Please fill in the Registration Type in customer")
            frappe.log_error("LHDN requires proper Registration Type." \
                    "Please fill in the Registration Type in customer")
            return
        if(not company_doc.company_registration or company_doc.company_registration == "null" or company_doc.company_registration == "None"):
            frappe.msgprint("LHDN requires proper Registration No." \
                    "Please fill in the Registration No in customer")
            frappe.log_error("LHDN requires proper Registration No." \
                    "Please fill in the Registration No in customer")
            return
        #BRN    
...
def customer_Data(invoice,sales_invoice_doc):
            try:
                ...
                if(not customer_doc.custom_registration_type):
                ...
                if int(frappe.__version__.split('.')[0]) == 13:
                    ...
                    if(not customer_doc.customer_primary_address):
                        frappe.msgprint("LHDN requires proper Address." \
                        "Please add your customer address in Customer")
                        frappe.log_error("LHDN requires proper Address." \
                        "Please add your customer address in Customer")
                        return
                ...
                #. / cac:Party / cac:PostalAddress / cbc:CityName
                if(not address.city or address.city == "null" or address.city == "None"):
                    cbc_CityName = ET.SubElement(cac_PostalAddress, "cbc:CityName")
                    cbc_CityName.text = None 
                else:
                    cbc_CityName = ET.SubElement(cac_PostalAddress, "cbc:CityName")
                ...
                cbc_CountrySubentity = ET.SubElement(cac_PostalAddress, "cbc:CountrySubentityCode")
                # cbc_CountrySubentity.text = "14"
                if(not address.custom_state_codes):
                    frappe.msgprint("LHDN requires proper State." \
                    "Please fill in the State in customer address")
                    frappe.log_error("LHDN requires proper State." \
                    "Please fill in the State in customer address")
                    return
                ...
                cbc_RegistrationName_1.text = sales_invoice_doc.customer_name
...
def doc_Reference(invoice,sales_invoice_doc,invoice_number):
            try:
                ...
                return invoice  
                    ...
                    frappe.throw("Error occured in  reference doc")
...
def tax_Data(invoice,sales_invoice_doc):
    try:
                ...
                if sales_invoice_doc.currency == "MYR":
                    ...
                    tax_amount_without_retention_myr =  round(abs(sales_invoice_doc.total_taxes_and_charges),2)
                    cbc_TaxAmount_MYR.text = str(round( tax_amount_without_retention_myr,2))     # str( abs(sales_invoice_doc.base_total_taxes_and_charges))
                ...
                cbc_PayableRoundingAmount.set("currencyID", sales_invoice_doc.currency)
                cbc_PayableRoundingAmount.text = str(sales_invoice_doc.rounding_adjustment)
                ...
                cbc_PayableAmount.text = str(round(abs(sales_invoice_doc.net_total) + abs(tax_amount_without_retention) + sales_invoice_doc.rounding_adjustment,2))
                return invoice
        ...
        for single_item in sales_invoice_doc.items:
            ...
            if single_item.custom_lhdn_tax_type_code == "E":  # lhdn Tax type  Exempted
                ...
                if(single_item.custom_tax_code):
                    item_tax_rate = frappe.get_doc("Sales Taxes and Charges Template", single_item.custom_tax_code)
...
def get_tax_total_from_items(sales_invoice_doc):
    return sales_invoice_doc.total_taxes_and_charges
